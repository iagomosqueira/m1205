...
This method is limited. The generated pairs of values of $r$ and $k$ might be
...
<<label=look_at_LLs_holes2,echo=FALSE, eval=FALSE>>=
plot(sort(c(ll_holes2)))
# scale between 0 and 1
# Ah, problem here. Negative probs that are never picked
# Make it so that the lowest ll has a prob of 0
ll_holes_shift <- ll_holes2 - min(ll_holes2)
probll <- c(ll_holes_shift / sum(ll_holes_shift))
plot(sort(probll))

# pick 5000 vals from here
rkpairs <- rep(NA,5000)
for (i in 1:5000)
	rkpairs[i] <- which(cumsum(probll)>=runif(1))[1]
fh2 <- iter(flsp_holes2_boot,rkpairs)
bc2 <- speedy_bcurrent(fh2)
ll2 <- ll(fh2)
# Compare the original lls and the resampled lls
par(mfrow=c(2,1))
plot(sort(c(ll_holes2)))
plot(sort(c(ll2)))
min(bc2)
# Still got some duds in there!


		# From MEC
		# Normalise the exinction probabilities so sum is 1
		#alldata$EPSim <- alldata$EPSim / sum(alldata$EPSim)
